...
p.addParamValue('debug', 0, @isscalar); % 0: no bug. 1: 'BgLum' bug (see history for 25.01.2022).
...
try
    ...
    if p.debug==1
        wnd = lpsy.openWindow(p.Screen, pp.BgLum,[0 0 600 400]); % DEBUG MODE NOW
    else 
        wnd = lpsy.openWindow(p.Screen, pp.BgLum); % FULL SCREEN MODE
    end
