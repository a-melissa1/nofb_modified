...
conditions_start_value = [100, 200, 200]; % OFFSET VALUE SHOULD BE CONSTANT THROUGHGOUT A SINGLE RUN
%ix = randperm(5);
%conditions_cond = conditions_cond(ix);
%conditions_start_value = conditions_start_value(ix);
 
conditions_ntrials = [10,10,10];
for m = [0, 1]
...
for cc = [1:length(conditions_cond)]
    ...
    p.mask = m;
    p.cond = cond;
    p.DvFilename = convertStringsToChars(fullfile('Data',strcat('nofb_noquest_', subjnr, '_cond',num2str(m), '_',num2str(cond),'.dv')));
